...
interface Ship {
  x: number; y: number;
  vx: number; vy: number;
  angle: number; thrusting: boolean; inv: number;
}
interface Rock {
  id: number; x: number; y: number;
  vx: number; vy: number;
  a: number; spin: number;
  sz: SZ; r: number; pts: V[];
}
interface Bullet {
  id: number; x: number; y: number;
...
function makePts(r: number): V[] {
  ...
  return Array.from({ length: n }, (_, i) => {
    ...
    return { x: Math.cos(a) * r * rr(0.6, 1.0), y: Math.sin(a) * r * rr(0.6, 1.0) };
  });
}
...
function makeRock(id: number, x: number, y: number, sz: SZ): Rock {
  ...
  return { id, x, y, sz, r, pts: makePts(r), vx: Math.cos(ang)*spd, vy: Math.sin(ang)*spd, a:0, spin: rr(-0.025,0.025) };
}
...
function spawnRocks(level: number, nid: number): [Rock[], number] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    let x = 0, y = 0;
    if      (edge === 0) { x = rr(0,CW); y = -70; }
    else if (edge === 1) { x = CW+70;    y = rr(0,CH); }
    else if (edge === 2) { x = rr(0,CW); y = CH+70; }
    else                 { x = -70;      y = rr(0,CH); }
...
function initGS(hi = 0): GS {
  ...
  return {
    phase: 'title',
    ship: { x:CW/2, y:CH/2, vx:0, vy:0, angle:-Math.PI/2, thrusting:false, inv:0 },
    rocks, bullets: [], particles: [],
    score:0, hi, lives:3, level:1, deadTimer:0, nid,
  };
}
...
export default function Asteroids({ onUnlock }: { onUnlock?: (level: number) => void }) {
  ...
  useEffect(() => {
    ...
    const explode = (gs: GS, x: number, y: number, n: number) => {
      for (let i=0; i<n; i++) {
        const a = Math.random()*Math.PI*2, spd = rr(0.8,3.5);
        const life = 25 + Math.floor(Math.random()*35);
        gs.particles.push({ x, y, vx:Math.cos(a)*spd, vy:Math.sin(a)*spd, life, maxLife:life });
      }
    };
    ...
    const drawShip = (s: Ship) => {
      ...
      if (s.thrusting) {
        ...
        ctx.shadowColor = '#ffd43b'; ctx.shadowBlur = 10; ctx.lineWidth = 1.2;
        ctx.beginPath();
    ...
    const drawRock = (rock: Rock) => {
      ctx.save();
      ctx.translate(rock.x, rock.y); ctx.rotate(rock.a);
      ctx.strokeStyle = COL; ctx.lineWidth = 1.5;
      ctx.shadowColor = COL; ctx.shadowBlur = 5;
      ctx.beginPath();
      rock.pts.forEach(({ x, y }, i) => i===0 ? ctx.moveTo(x,y) : ctx.lineTo(x,y));
    ...
    const drawMiniShip = (x: number, y: number) => {
      ...
      ctx.beginPath();
      ctx.moveTo(8,0); ctx.lineTo(-6,4.5); ctx.lineTo(-3,0); ctx.lineTo(-6,-4.5);
      ctx.closePath(); ctx.stroke();
      ctx.restore();
    };
    ...
    const draw = (gs: GS) => {
      ...
      // Scanlines
      for (let y=0; y<CH; y+=4) { ctx.fillStyle='rgba(0,0,0,0.055)'; ctx.fillRect(0,y,CW,2); }
      ...
      if (gs.phase === 'title') {
        ...
        ctx.shadowBlur = 0;
        ctx.font = '12px monospace'; ctx.fillStyle = '#2d5a38';
        ctx.fillText('ARROWS / WASD  to move  ·  SPACE  to fire', CW/2, CH/2 + 6);
      ...
      for (const b of gs.bullets) { ctx.beginPath(); ctx.arc(b.x,b.y,2.5,0,Math.PI*2); ctx.fill(); }

      // Ship
      if (gs.phase === 'play') drawShip(gs.ship);

      // HUD
      ctx.shadowBlur = 0; ctx.font = '13px monospace'; ctx.fillStyle = COL;
      ctx.textAlign = 'left';  ctx.fillText(`SCORE  ${gs.score}`, 14, 22);
      ...
      if (gs.phase === 'over') {
        ...
        ctx.textAlign='center'; ctx.shadowColor=COL; ctx.shadowBlur=20; ctx.fillStyle=COL;
        ctx.font='bold 42px monospace'; ctx.fillText('GAME OVER', CW/2, CH/2-52);
        ctx.shadowBlur=0; ctx.font='15px monospace'; ctx.fillStyle='#4a7a55';
        ctx.fillText(`SCORE: ${gs.score}    HI: ${gs.hi}`, CW/2, CH/2+2);
        ctx.shadowColor=COL; ctx.shadowBlur=10; ctx.fillStyle=COL; ctx.font='13px monospace';
        if (Math.floor(Date.now()/560)%2===0) ctx.fillText('[ PRESS SPACE TO PLAY AGAIN ]', CW/2, CH/2+60);
      }
    };
    ...
    const tick = () => {
      ...
      if (gs.phase === 'play') {
        ...
        s.x=wrap(s.x+s.vx,CW); s.y=wrap(s.y+s.vy,CH);
        if (s.inv>0) s.inv--;

        gs.bullets = gs.bullets.filter(b => {
          b.x=wrap(b.x+b.vx,CW); b.y=wrap(b.y+b.vy,CH); return --b.life>0;
        });

        gs.particles = gs.particles.filter(p => {
          p.x+=p.vx; p.y+=p.vy; p.vx*=0.97; p.vy*=0.97; return --p.life>0;
        });
        ...
        for (const b of gs.bullets) {
          ...
          for (const r of gs.rocks) {
            ...
            if (Math.hypot(b.x-r.x, b.y-r.y) < r.r) {
              dead.add(r.id); b.life=0;
              gs.score += SCORES[r.sz];
              explode(gs, r.x, r.y, r.sz==='L'?14:r.sz==='M'?9:5);
              if (r.sz==='L') { newRocks.push(makeRock(gs.nid++,r.x,r.y,'M')); newRocks.push(makeRock(gs.nid++,r.x,r.y,'M')); }
              if (r.sz==='M') { newRocks.push(makeRock(gs.nid++,r.x,r.y,'S')); newRocks.push(makeRock(gs.nid++,r.x,r.y,'S')); }
              break;
            }
          }
        }
      ...
      if (gs.phase === 'dead') {
        ...
        if (--gs.deadTimer<=0) {
          gs.phase='play';
          gs.ship={ x:CW/2, y:CH/2, vx:0, vy:0, angle:-Math.PI/2, thrusting:false, inv:INVINCIBLE };
          gs.bullets=[];
        }
      }
